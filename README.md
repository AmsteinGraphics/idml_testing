...
| `watch_indesign.ps1` | mirror saved `.indd`/`.idml` from a local Windows folder into the repo. Optional, and only for the Windows + WSL setup: InDesign opens documents [Read-Only] from `\\wsl.localhost` because it cannot reliably take its lock over a network path, so its working copy has to live on a local disk | run on Windows |
